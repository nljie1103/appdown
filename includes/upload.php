<?php
/**
 * 文件上传处理
 */

function resolve_filename_collision(string $dir, string $base, string $ext): string {
    $filename = $base . '.' . $ext;
    if (!file_exists($dir . '/' . $filename)) return $filename;
    $i = 1;
    do {
        $filename = $base . '(' . $i . ').' . $ext;
        $i++;
    } while (file_exists($dir . '/' . $filename));
    return $filename;
}

function handle_upload(string $field, string $category, string $custom_name = ''): array {
    if (empty($_FILES[$field]) || $_FILES[$field]['error'] !== UPLOAD_ERR_OK) {
        $code = $_FILES[$field]['error'] ?? -1;
        return ['ok' => false, 'error' => "上传失败 (code: $code)"];
    }

    $file = $_FILES[$field];
    $rules = get_upload_rules($category);
    if (!$rules) return ['ok' => false, 'error' => '无效的上传类型'];

    if ((int)$file['size'] <= 0) return ['ok' => false, 'error' => '文件为空'];
    if ($file['size'] > $rules['max_size']) {
        $max_mb = round($rules['max_size'] / 1024 / 1024, 1);
        return ['ok' => false, 'error' => "文件大小超过限制 ({$max_mb}MB)"];
    }

    $ext = strtolower(pathinfo($file['name'], PATHINFO_EXTENSION));
    if (!in_array($ext, $rules['extensions'], true)) {
        return ['ok' => false, 'error' => '不支持的文件类型，允许: ' . implode(', ', $rules['extensions'])];
    }

    if ($category === 'image') {
        $finfo = finfo_open(FILEINFO_MIME_TYPE);
        $mime = finfo_file($finfo, $file['tmp_name']);
        finfo_close($finfo);
        $allowedMimes = ['image/webp', 'image/png', 'image/jpeg', 'image/gif', 'image/x-icon', 'image/vnd.microsoft.icon'];
        if (!in_array($mime, $allowedMimes, true)) {
            return ['ok' => false, 'error' => "文件MIME类型不合法: $mime"];
        }
        if ($ext !== 'ico' && @getimagesize($file['tmp_name']) === false) {
            return ['ok' => false, 'error' => '图片文件无法解析'];
        }
    }

    // APK/IPA 不再只看扩展名，必须满足实际容器结构。
    if ($category === 'app' && in_array($ext, ['apk', 'ipa'], true)) {
        $check = validate_app_archive($file['tmp_name'], $ext);
        if (!$check['ok']) return $check;
    }

    if ($custom_name !== '') {
        $clean = preg_replace('/[^\w\x{4e00}-\x{9fff}.\-]/u', '_', $custom_name);
        $safe_name = $clean . '.' . $ext;
    } else {
        $original = pathinfo($file['name'], PATHINFO_FILENAME);
        $clean = preg_replace('/[^\w\x{4e00}-\x{9fff}.\-]/u', '_', $original);
        if ($clean === '' || $clean === '_') $clean = time() . '_' . bin2hex(random_bytes(4));
        $safe_name = $clean . '.' . $ext;
    }

    $dest_dir = __DIR__ . '/../uploads/' . $category . 's';
    if (!is_dir($dest_dir) && !mkdir($dest_dir, 0755, true) && !is_dir($dest_dir)) {
        return ['ok' => false, 'error' => '无法创建上传目录'];
    }

    $dest_path = $dest_dir . '/' . $safe_name;
    if (file_exists($dest_path)) {
        $base = pathinfo($safe_name, PATHINFO_FILENAME);
        $safe_name = resolve_filename_collision($dest_dir, $base, $ext);
        $dest_path = $dest_dir . '/' . $safe_name;
    }
    if (!move_uploaded_file($file['tmp_name'], $dest_path)) return ['ok' => false, 'error' => '文件保存失败'];
    @chmod($dest_path, in_array($category, ['cert', 'keystore'], true) ? 0600 : 0644);

    $url = 'uploads/' . $category . 's/' . $safe_name;
    return ['ok' => true, 'url' => $url, 'filename' => $safe_name];
}

function get_upload_rules(string $category): ?array {
    $php_max = min(parse_size(ini_get('upload_max_filesize')), parse_size(ini_get('post_max_size')));
    if ($php_max <= 0) $php_max = PHP_INT_MAX;
    $limit = fn(int $bytes): int => min($php_max, $bytes);

    return match ($category) {
        'image' => ['extensions' => ['webp', 'png', 'jpg', 'jpeg', 'gif', 'ico'], 'max_size' => $limit(20 * 1024 * 1024)],
        'font' => ['extensions' => ['ttf', 'woff', 'woff2', 'otf'], 'max_size' => $limit(20 * 1024 * 1024)],
        'app' => ['extensions' => ['apk', 'ipa', 'exe', 'dmg', 'zip'], 'max_size' => $php_max],
        'cert' => ['extensions' => ['pem', 'crt', 'key', 'p12'], 'max_size' => $limit(5 * 1024 * 1024)],
        'keystore' => ['extensions' => ['jks', 'keystore', 'p12', 'pfx', 'bks'], 'max_size' => $limit(10 * 1024 * 1024)],
        default => null,
    };
}

function parse_size(string $val): int {
    $val = trim($val);
    if ($val === '') return 0;
    $unit = strtolower(substr($val, -1));
    $num = (float)$val;
    $bytes = match ($unit) {
        'g' => $num * 1024 * 1024 * 1024,
        'm' => $num * 1024 * 1024,
        'k' => $num * 1024,
        default => $num,
    };
    return (int)min($bytes, PHP_INT_MAX);
}

function delete_upload(string $url): void {
    if (substr($url, 0, 8) !== 'uploads/' || strpos($url, '..') !== false) return;
    $uploadsDir = realpath(__DIR__ . '/../uploads');
    if (!$uploadsDir) return;
    $path = realpath(__DIR__ . '/../' . $url);
    if ($path && str_starts_with($path, $uploadsDir . DIRECTORY_SEPARATOR) && is_file($path)) unlink($path);
}
