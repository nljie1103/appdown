...
const AlertModal = {
    ...
    show(type, msg, detail) {
        ...
        document.getElementById('_alertActions').innerHTML =
            '<button class="btn btn-primary" style="min-width:120px;" onclick="AlertModal.hide()">关闭</button>';
        this._resolve = null;
    ...
    /**
     * 确认弹窗（复用 AlertModal UI）
     * @param {string} msg 主提示语
     * @param {string} detail 详细描述（可选）
     * @param {object} opts  { okText, okClass, icon }
     * @returns {Promise<boolean>}
     */
    confirm(msg, detail, opts) {
        this._ensure();
        opts = opts || {};
        const iconType = opts.icon || 'warning';
        const icons = {
            warning: '<span style="color:#f59e0b;">&#9888;</span>',
            danger:  '<span style="color:#ef4444;">&#10008;</span>',
            info:    '<span style="color:#3b82f6;">&#8505;</span>',
        };
        document.getElementById('_alertIcon').innerHTML = icons[iconType] || icons.warning;
        document.getElementById('_alertMsg').textContent = msg || '确定要执行此操作吗？';
        document.getElementById('_alertDetail').textContent = detail || '';
        const okClass = opts.okClass || 'btn-primary';
        const okText = opts.okText || '确定';
        document.getElementById('_alertActions').innerHTML =
            `<button class="btn btn-outline" style="min-width:80px;" onclick="AlertModal.hide()">取消</button>` +
            `<button class="btn ${okClass}" style="min-width:80px;margin-left:12px;" onclick="AlertModal._ok()">` + okText + `</button>`;
        this._overlay.classList.add('active');
