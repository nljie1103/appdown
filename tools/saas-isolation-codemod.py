...
# IPA worker. Keep the build workspace under the shared Docker mount, but namespace it by tenant.
replace('tools/ios-build-worker.php', "require_once __DIR__ . '/../includes/db.php';", "require_once __DIR__ . '/../includes/saas.php';\nrequire_once __DIR__ . '/../includes/db.php';")
replace(
    'tools/ios-build-worker.php',
...
# Certificate resolver: SaaS tenants may only read certificate files from their own upload root.
old_cert = """        case 'path':
            // 仅允许受信任位置：项目目录、/etc/ssl、/etc/pki
            // 注意：管理员可以指定系统证书目录路径（如 Let's Encrypt）
            ...
            foreach ($allowedRoots as $root) {
                if (str_starts_with($realPath, $root . DIRECTORY_SEPARATOR) || $realPath === $root) {
                    return is_file($realPath) ? (string)file_get_contents($realPath) : '';
                }
            }
            return '';
        case 'upload':
            // 只允许项目 uploads 目录下的文件，禁止路径穿越
            $clean = ltrim(str_replace('\\\\', '/', $value), '/');
            if (!str_starts_with($clean, 'uploads/') || strpos($clean, '..') !== false) return '';
            $path = realpath(__DIR__ . '/../' . $clean);
            $uploadRoot = realpath(__DIR__ . '/../uploads');
            if (!$path || !$uploadRoot || !str_starts_with($path, $uploadRoot . DIRECTORY_SEPARATOR)) return '';
...
# Image/attachment physical rename keeps the tenant prefix.
...
# Package inspector must never inspect another tenant's file by a crafted URL.
replace(
    'admin/api/package-info.php',
...
# Tenant-only backup and cross-slug restore remapping.
...
# Maintenance is tenant scoped. Shared Gradle/iOS environment caches remain outside the tenant lifecycle.
replace('tools/maintenance-core.php', "require_once __DIR__ . '/../includes/db.php';", "require_once __DIR__ . '/../includes/saas.php';\nrequire_once __DIR__ . '/../includes/db.php';")
replace(
    'tools/maintenance-core.php',
...
# Remove staging helpers from the resulting SaaS branch.
