...
$SSH_HOST = 'localhost';
$SSH_USER = 'user';
$knownHosts = $projectRoot . '/data/ios_known_hosts';
...
try {
    ...
    if ($result['code'] !== 0 || trim($result['output'][0] ?? '') !== 'ok') {
        fail_task($pdo, $taskId, "无法通过 SSH 连接到 macOS 容器\n请确认 Docker-OSX 容器正在运行；若容器重建导致主机指纹变化，请删除 data/ios_known_hosts 后重试");
        exit(1);
    }
    ...
    $versionName = preg_replace('/[^0-9A-Za-z._-]/', '', (string)($params['version_name'] ?? '1.0.0')) ?: '1.0.0';
    $versionCode = (string)max(1, (int)($params['version_code'] ?? 1));
    $appName = str_replace(["\\", '"', "\r", "\n"], ['\\\\', '\\"', ' ', ' '], (string)$params['app_name']);
    ...
    if ($appName !== '') {
        $pbxContent = str_replace('PRODUCT_NAME = "$(TARGET_NAME)"', 'PRODUCT_NAME = "' . $appName . '"', $pbxContent);
    }
    ...
    $xcodeCmd = "cd $remoteBuildDir && " .
        ...
        "-archivePath build/app.xcarchive archive CODE_SIGNING_ALLOWED=NO 2>&1";
    $sshFullCmd = 'ssh ' . $SSH_OPTS . ' ' . escapeshellarg($SSH_USER . '@' . $SSH_HOST) . ' ' . escapeshellarg($xcodeCmd);
    ...
    while (true) {
        while (($line = fgets($pipes[1])) !== false) {
            $line = rtrim($line);
            if ($line === '') continue;
            $output[] = redact_sensitive_text($line);
            $newPct = parse_xcode_progress($line, $lastProgress);
            if ($newPct > $lastProgress) {
                $lastProgress = $newPct;
                update_task($pdo, $taskId, ['progress' => $newPct, 'progress_msg' => xcode_progress_msg($newPct)]);
            }
        }
        while (($line = fgets($pipes[2])) !== false) $output[] = redact_sensitive_text(rtrim($line));
    ...
    while (($line = fgets($pipes[1])) !== false) $output[] = redact_sensitive_text(rtrim($line));
    while (($line = fgets($pipes[2])) !== false) $output[] = redact_sensitive_text(rtrim($line));
    ...
    $packCmd = "cd $remoteBuildDir/build && mkdir -p Payload && cp -r app.xcarchive/Products/Applications/*.app Payload/ && zip -r -q app.ipa Payload/";
    ...
    $stmt->execute([$taskId, $params['app_name'], $params['bundle_id'] ?? 'com.example.webviewapp', $params['version_name'] ?? '1.0.0', (int)($params['version_code'] ?? 1), $params['url'], $params['icon_url'] ?? '', $ipaUrl, $ipaSize, 'unsigned']);
    update_task($pdo, $taskId, ['status' => 'done', 'progress' => 100, 'progress_msg' => '构建完成', 'result_url' => $ipaUrl, 'result_size' => $ipaSize]);
...
function parse_xcode_progress(string $line, int $current): int {
    ...
    if (strpos($line, 'ARCHIVE SUCCEEDED') !== false || strpos($line, 'Archive Succeeded') !== false || strpos($line, 'BUILD SUCCEEDED') !== false || strpos($line, '** ARCHIVE SUCCEEDED **') !== false) return 80;
    return $current;
}
...
function resize_image(string $src, string $dst, int $w, int $h): void {
    ...
    $srcImg = match ($info['mime']) {
        'image/png' => @imagecreatefrompng($src), 'image/jpeg' => @imagecreatefromjpeg($src),
        'image/gif' => @imagecreatefromgif($src), 'image/webp' => @imagecreatefromwebp($src), default => null,
    };
    if (!$srcImg) { copy($src, $dst); return; }
    $dstImg = imagecreatetruecolor($w, $h); imagealphablending($dstImg, false); imagesavealpha($dstImg, true);
